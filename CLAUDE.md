...
- **「全国安排」头像机器人系**(2026-05 观察) — 招嫖话术「全国安排」只画在**头像图片**里(脚本读不到图片文字),显示名是随机字母乱码(如 `Hqzbrc`/`Qnegk`/`Bihysuq`),句柄 = 显示名小写 + 随机数字(`@hqzbrc85482`),推文是无害随机英文(音乐/心碎)+ emoji。**关键词完全失效**。靠 `calculateSpamScore()` **规则 6** 的结构启发式:显示名 `^[A-Za-z]{5,12}$` 且 句柄小写 == 显示名小写 + `\d{5,}`(**5位以上随机数字**) 且 名字是乱码(`元音比 ≤ 0.2` 或(`最长辅音串 ≥ 3` 且 `元音比 < 0.3`))。已验证抓全样本、放过 Brandon/Andrew/Steven/Kevin 等真名。这是 **auto-hide**。
  - **误杀教训(v0.13.2)**:乱码统计判据与真实短名/拼音拼接名**本质重叠** —— `small`(元音比恰好 0.2)、`NickSung`(辅音串 `cks`=3 且元音比 0.25) 都会命中"乱码"判定。原来句柄数字门槛是 `\d{3,}`,把 `@NickSung2017`/`@small2555`(后缀是 **4 位年份**)也吃进去自动隐藏了。修复:数字门槛提到 `\d{5,}` —— bot 用 5 位随机数(`@hqzbrc85482`),真名后缀多是 4 位年份,**句柄数字位数是独立于名字统计的强信号**,这一刀救回真名且不削弱已知 bot 捕获。注意取舍:若某 bot 改用 3-4 位数字后缀会被放过(走人工/关键词/票数兜底),符合「宁可漏网走人工,也别误杀」。
