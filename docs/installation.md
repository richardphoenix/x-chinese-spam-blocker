...
- **提交此账号到黑名单**：将当前账号信息提交到后台审核队列，提交成功后显示 toast 提示

## 4. 推荐用法

1. 日常使用只开「隐藏」即可，体验最好，也最安全
2. 偶尔看到明显 spam 时点击「提交此账号到黑名单」，脚本会自动提交到后台审核队列，无需离开当前页面
...
- 想贡献新的 spam 账号：使用脚本里的「提交此账号到黑名单」按钮，账号信息会提交到后台审核队列，维护者审核通过后自动写入黑名单；也可直接编辑 `blocklist/blocklist.json` 提 PR
